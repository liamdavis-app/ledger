...
- **Progress tab**: stats grid, headline insight card (`headlineInsight()`), collapsible accordion sections (state persisted to `db.prefs.accOpen`), timeframe selector (4wk/12wk/all-time), Lift explorer (split/muscle/search filters, 4 chart modes incl. kg/rep, SVG line chart with projection), Personal bests, Muscle group balance, Training frequency heatmap, weekly report card (Mon–Sun, dismissable, split + exercise-level deltas).
